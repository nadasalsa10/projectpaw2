...
<body class="bg-gray-900 text-gray-100 flex flex-col min-h-screen pb-20">
    ...
    <!-- Main Content -->
    <main class="flex-grow max-w-md mx-auto w-full px-4 pt-4">
        @yield('content')
    </main>

    <!-- Driver Bottom Navigation Bar -->
    <div class="fixed bottom-0 left-0 right-0 bg-gray-800 border-t border-gray-700 py-2 px-4 flex justify-around items-center z-40 shadow-2xl">
        <a href="{{ route('driver.dashboard') }}" class="flex flex-col items-center {{ request()->routeIs('driver.dashboard') ? 'text-emerald-400 font-bold' : 'text-gray-400' }}">
            <i class="fa-solid fa-gauge text-lg"></i>
            <span class="text-[10px] mt-1">Dashboard</span>
        </a>
        <a href="{{ route('driver.schedules') }}" class="flex flex-col items-center {{ request()->routeIs('driver.schedules*') ? 'text-emerald-400 font-bold' : 'text-gray-400' }}">
            <i class="fa-solid fa-calendar-days text-lg"></i>
            <span class="text-[10px] mt-1">Jadwal</span>
        </a>
        <a href="{{ route('driver.scan') }}" class="flex flex-col items-center relative -top-3">
            <div class="w-12 h-12 bg-emerald-600 rounded-full flex items-center justify-center text-white shadow-lg border-4 border-gray-900">
                <i class="fa-solid fa-qrcode text-xl"></i>
            </div>
            <span class="text-[10px] mt-0.5 font-bold text-emerald-400">Scan QR</span>
        </a>
        <a href="{{ route('driver.reports') }}" class="flex flex-col items-center {{ request()->routeIs('driver.reports') ? 'text-emerald-400 font-bold' : 'text-gray-400' }}">
            <i class="fa-solid fa-file-invoice text-lg"></i>
            <span class="text-[10px] mt-1">Laporan</span>
        </a>
        <a href="{{ route('driver.profile') }}" class="flex flex-col items-center {{ request()->routeIs('driver.profile') ? 'text-emerald-400 font-bold' : 'text-gray-400' }}">
